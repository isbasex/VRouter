...
class VRouterRemote {
  ...
  remoteExec(cmd) {
    const specialCmds = ['/etc/init.d/firewall restart']
  ...
  installKt() {
    const cmd =
      `tar -xvzf ${this.config.vrouter.configDir}/third_party/kcptun*.tar.gz ` +
  ...
  getOpenwrtVersion() {
    ...
    return this.remoteExec(cmd).then(output => {
      const reg = /^ *(\w+ \w+ \(.*\)) *$/gm
  ...
  async changeMode(m, p) {
    ...
    await Promise.all([this.restartFirewall(), this.restartDnsmasq()])
